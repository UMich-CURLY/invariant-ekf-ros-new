...
void InEKF_ROS::initState() {
    ...
    if (initialize_state_from_first_observation_) {
        ...
        while (!p_initialized) {
            // Try next item (Blocking)
            m_queue_.pop(m_ptr);
            ROS_DEBUG("Initialize position from kinematics or landmark measurement");
            switch (m_ptr->getType()) {
                case CONTACT: {
                    auto contacts = dynamic_pointer_cast<ContactMeasurement>(m_ptr);
                    filter_.setContacts(contacts->getData());
                    ROS_DEBUG("get contact type for init");
                    break;
                }
                case KINEMATIC: {
                  ROS_DEBUG("get kin type for init");
               ...
               default: {
                 ROS_DEBUG_STREAM("init msg type unknown: "<<m_ptr->getType());
               }
            }
...
void InEKF_ROS::kinematicsCallback(const inekf_msgs::KinematicsArray::ConstPtr& msg) {
    ...
    ROS_DEBUG("Push kinematics to m_queue");    
    m_queue_.push(ptr);
}
...
void InEKF_ROS::contactCallback(const inekf_msgs::ContactArray::ConstPtr& msg) {
    ...
    ROS_DEBUG("Push Contact to m_queue");
    m_queue_.push(ptr);
}
